                        ...
                        <div class="row g-1">
                         ...
                         <div class="d-grid  p-1 col-md-auto col-sm-12 order-3 btn-responsive">
                            <?php if (chk_para(session("user_info")["ui_para"],$menu_active."_add")=="ok" or session("user_info")["ui_type"]=="1"){ ?>
                              <a class="btn btn-primary btn-responsive" data-bs-toggle="modal" data-remote="{{ route('car.create') }}"  data-bs-target="#modal-add"><i class="fas fa-plus"></i>
                          ...
                          <?php } ?>
                            ...
                            <div class="d-grid p-1 col-md-auto col-sm-12 order-2 btn-responsive">
                                <form action="{{ route('car.list') }}" method="GET">
                                    <div class="input-group">
                                        ...
                                        <input name="q" type="text" class="form-control" placeholder="جزء من الاسم أو الموديل أو البراند">
                                        <button class="btn btn-info" type="submit" id="button-addon2">
                                            <?php if(isset($_REQUEST["q"]) and $_REQUEST["q"]!=="" ){echo "عرض الكل";}else{echo "بحث";} ?></button>
                                        </div>
                                </form>
                            </div>
                            <div class="d-grid p-1 col-md-auto col-sm-12 order-1 btn-responsive">
                                <?php if (chk_para(session("user_info")["ui_para"],$menu_active."_rep")=="ok" or session("user_info")["ui_type"]=="1"){ ?>
                                    <a class="btn btn-success btn-responsive" data-bs-toggle="modal" data-remote="{{ route('car.rep') }}"  data-bs-target="#modal-rep"><i class="far fa-file-alt"></i> <b>التقارير</b></a>
                                  <?php } ?>
                                  </div>
                                ...
                                <table class="table app-table-hover mb-0 table-striped table-bordered text-center" data-toggle="table" id="datat_list">
                                    <thead>
                                        <tr class="bg-infox">
                                            ...
                                            <th class="text-center" data-visible="true">البراند</th>
                                            <th class="text-center" data-visible="true">الموديل</th>
                                            <th class="text-center" data-visible="true">السنة</th>
                                            <th class="text-center" data-visible="true">السعر اليومي</th>
                                            <th class="text-center" data-visible="true">التوفر</th>
                                            <th class="text-center" data-visible="true">الصورة</th>
                                            <th class="text-center" data-visible="true">الموقع</th>
                                    ...
                                    <tbody>
                                        @unless ($listings->isEmpty())
                                            @foreach ($listings as $car)
                                                <tr>
                                                    <td class="cell">
                                                        <a class="btn btn-success" data-bs-toggle="modal" data-remote="{{ route('car.show', enc($car->id)) }}" data-bs-target="#modal-show">
                                                            <i class="fas fa-file-alt"></i>
                                                        </a>
                                                    </td>
                                                    <td class="cell">{{ $car->name }}</td>
                                                    <td class="cell">{{ $car->brand }}</td>
...
<script>
    function del(vall) {
        if (vall == "") {
            swal({ text: "لا يوجد شيء لحذفه", icon: "error", timer: 2000, button: false });
            return false;
        }
        swal({
            title: "تأكيد الحذف",
            text: "هل ترغب بالتأكيد حذف السجل؟",
            icon: "warning",
            dangerMode: true,
            buttons: ["لا", "نعم"],
        }).then((willDelete) => {
            if (willDelete) {
                window.swal({ title: "جاري الحذف", text: "الرجاء الانتظار", icon: "{{ asset('images/loding2.gif') }}", buttons: false, allowOutsideClick: false });
                axios.delete(`/car/${vall}`)
                    .then(response => {
